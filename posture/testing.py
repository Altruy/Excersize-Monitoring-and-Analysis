...
def check_position(landmarks):
    ...
    if hip_angle > 160 and knee_angle > 160:
        return "Standing"
    elif hip_angle < 60 and knee_angle < 100:
        return "Lowered"
    else:
...
def evaluate_lowered_position(landmarks):
    ...
    ankle = get_landmark_coords(landmarks, 'LEFT_ANKLE')
    shoulder = get_landmark_coords(landmarks, 'LEFT_SHOULDER')
    toe = get_landmark_coords(landmarks, 'LEFT_FOOT_INDEX')

    hip_angle = calculate_angle(shoulder, hip, knee)
    knee_angle = calculate_angle(hip, knee, ankle)
    
    # Check if knees are aligned with toes
    knee_to_toe_distance = abs(knee[0] - toe[0])
    ...
    if knee_to_toe_distance > 0.05:  # Adjust threshold as necessary
        feedback += " and keep your knees aligned with your toes"
